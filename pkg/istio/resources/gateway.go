...
//MakeIstioGateway is
func MakeIstioGateway(virtualappconfig api.VirtualAppConfig, namespace string) (istioapi.Gateway, string) {
	istioServers := make([]istioapi.Server, 0)

	for _, server := range virtualappconfig.Spec.Servers {
	...
	gatewayName := Prefix + virtualappconfig.Name + GatewaySuffix

	gateway := makeGateway(gatewayName, gatewaySpec, virtualappconfig)

	return gateway, gatewayName
}

func makeGateway(name string, gatewaySpec istioapi.GatewaySpec, virtualappconfig api.VirtualAppConfig) istioapi.Gateway {
	return istioapi.Gateway{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: virtualappconfig.Namespace,
